...
/**
 * 初始化导入所有数据
 * ⚠️ 注意：此函数仅在云端无数据时作为兜底使用
 * 云端数据为权威数据源，硬编码数据不应覆盖云端数据
 */
export function initializeData(): { staffCount: number; studentCount: number } {
  ...
  // 如果云端数据已下载到本地，不用硬编码数据覆盖
  const cloudStaff = JSON.parse(localStorage.getItem('kt_staff') || '[]');
  const cloudStudents = JSON.parse(localStorage.getItem('kt_students') || '[]');
  if (cloudStaff.length > 10 && cloudStudents.length > 10) {
    console.log(`[DataInit] 云端数据已存在(${cloudStaff.length}教职工, ${cloudStudents.length}学生)，跳过硬编码初始化`);
