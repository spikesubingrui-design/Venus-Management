...
// 转换教职工数据为系统格式
export function convertStaffData() {
  return staffData.map((staff, index) => ({
    id: generateId('staff'),
    name: staff.name,
    role: staff.role,
    phone: staff.phone,
    avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(staff.name)}`,
...
// 转换学生数据为系统格式
export function convertStudentData() {
...
// 导出数据供直接使用
export const STAFF_DATA = convertStaffData();
export const STUDENT_DATA = convertStudentData();
